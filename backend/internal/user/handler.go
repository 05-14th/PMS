...
// Handler manages HTTP routes for the device gateway.
// Service is kept so existing wiring does not break, but is unused here.
type Handler struct {
	service *Service
}
...
// Device tracks last known state of a node.
type Device struct {
	ID          string
	IP          string
	LastHello   time.Time
...
// RegisterRoutes sets up the watering gateway routes on the shared chi router.
func (h *Handler) RegisterRoutes(router chi.Router) {
	...
	router.Post("/command/{id}", postCommand)
	router.Get("/next", getNext)
	router.Post("/ack", postAck)

	// Feeder control passthrough
	router.Post("/push/{id}", postPush)

	// Watering system control passthroughs
	router.MethodFunc(http.MethodGet, "/mode/{id}", modeHandler)
	router.MethodFunc(http.MethodPost, "/mode/{id}", modeHandler)

	// Accept OPTIONS for set relays (CORS preflight)
	router.MethodFunc(http.MethodOptions, "/set-relays/{id}", setRelaysHandler)
	router.MethodFunc(http.MethodPost, "/set-relays/{id}", setRelaysHandler)

	// Cached telemetry and live status view
	router.Get("/telemetry/{id}", getTelemetryCached)
	router.Get("/status/{id}", getStatusPassthrough)
}
...
// Command queue compatibility: POST /command/{id}
...
// GET /next returns all queued commands for a device specified via query ?dev=id
