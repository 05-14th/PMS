...
import { DeleteOutlined, PlusOutlined, EditOutlined } from '@ant-design/icons'; // <-- MODIFIED
import dayjs from 'dayjs';
import axios from 'axios';
import AddCustomerForm from '../Forms_Sales/AddCustomerForm';
import EditSales from '../Forms_Sales/EditSales';
...
interface SaleItem {
    ...
    SaleDate: string;
    subtotal: number; // <-- MODIFIED (Added subtotal property)
}
...
const NewSale: React.FC = () => {
    ...
    const [isEditModalVisible, setIsEditModalVisible] = useState(false); // <-- ADDED
    const [currentItemToEdit, setCurrentItemToEdit] = useState<SaleItem | null>(
        null
    ); // <-- ADDED
    ...
    const onFinish = (values: any) => {
        ...
        const addItemToOrder = () => {
            const newSaleItem: SaleItem = {
                ...
                subtotal: values.TotalWeightKg * values.PricePerKg,
            };
        ...
        const isFinalSaleOfBatch =
            ...
            values.QuantitySold === availableQty;
    ...
    const handleEditClick = (record: SaleItem) => {
        setCurrentItemToEdit(record);
    ...
    const handleSaveSubtotal = (newSubtotal: number) => {
        if (!currentItemToEdit) return;

        setSalesItems((prevItems) =>
            prevItems.map((item) =>
                item.key === currentItemToEdit.key
                    ? { ...item, subtotal: newSubtotal }
                    : item
            )
        );
        handleCancelEdit(); // Close modal on save
        message.success('Subtotal updated!');
    };
    ...
    const handleAddCustomer = async (values: any) => {
        try {
            ...
            message.success(
                'Customer added successfully! You can now select them.'
            );
    ...
    const weightValidator = (_: any, value: number) => {
        ...
        const availableWeight =
            selectedProduct.WeightRemainingKg - weightInCart;
    ...
    const orderItemColumns = [
        ...
        {
            title: 'Subtotal (₱)',
            dataIndex: 'subtotal', // <-- MODIFIED (Use dataIndex for clarity)
            key: 'subtotal',
            render: (subtotal: number) => `₱${subtotal.toFixed(2)}`, // <-- MODIFIED (Render the stored subtotal)
        },
        {
            title: 'Actions',
            key: 'actions',
            render: (
                _: any,
                record: SaleItem // <-- MODIFIED (Added Space and Edit Button)
            ) => (
                <Space>
                    <Button
                        type='link'
    ...
    const calculateTotal = () =>
        // <-- MODIFIED (Sum up stored subtotals)
        salesItems.reduce((sum, item) => sum + item.subtotal, 0);
    ...
    return (
        <div className='p-2 sm:p-4'>
            {/* Cards and Forms remain the same */}
            <Card
                title={
                    <Title
                        level={4}
                ...
                styles={{ body: { padding: screens.xs ? '8px' : '16px' } }}
            >
                <Form
                    form={form}
                    layout='vertical'
