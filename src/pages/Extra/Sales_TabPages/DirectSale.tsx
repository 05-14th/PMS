    ...
    const handleSaveSale = async () => {
        ...
        try {
            ...
            const payload = {
                ...
                saleDate: dayjs(orderInfo.SaleDate).format(
                    'YYYY-MM-DD HH:mm:ss'
                ),
            ...
            const mappedProducts = (productsRes.data || []).map(
                (product: any) => ({
                    harvestProductID:
                        product.harvestProductID || product.HarvestProductID,
                    harvestDate: product.harvestDate || product.HarvestDate,
                    ...
                    <>
                        ...
                        >
                            ...
                            >
                                ...
                                >
                                    ...
                                    >
                                        {availableProducts.map((p) => (
                                            ...
                                            >
                                                {p.productType} - {p.batchName}{' '}
                                                (Qty: {p.quantityRemaining}, Wt:{' '}
                                                {p.weightRemainingKg.toFixed(2)}
                                                kg)
                                            </Option>
                                        ))}
                                    </Select>
                                            ...
                                            validator: (_, value) => {
                                                if (
                                                    ...
                                                    return Promise.reject(
                                                        new Error(
                                                            `Max ${availableForSale} available`
                                                        )
                                                    );
                                                }
                                ...
                                {selectedProduct && (
                                    <Tag color='blue'>
                                        Available: {availableForSale} pcs
                                    </Tag>
                                )}
                            </Space>
                        </Form>
