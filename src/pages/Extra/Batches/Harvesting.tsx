...
const Harvesting: React.FC<HarvestingProps> = ({ batch, onDataChange }) => {
    ...
    const handleAddNewType = async () => {
        ...
        try {
            ...
            message.success(
                `Product type "${newTypeName}" added successfully.`
            );
            setNewTypeName('');
            await fetchProductTypes(); // Refresh the list
        } catch (error: any) {
            const errorMsg =
                error.response?.data?.error || 'Failed to add new type.';
            message.error(errorMsg);
        }
    };
    ...
    return (
        <div className='space-y-8'>
                ...
                >
                    ...
                    {productType === 'Live' && (
                        ...
                        >
                            ...
                            <Checkbox
                                onChange={(e) =>
                                    setCreateInstantSale(e.target.checked)
                                }
                            >
                                {' '}
            ...
            <div className='text-center'>
                <Button
                    onClick={() => setIsProcessingModalVisible(true)}
                    size='large'
                >
                                    ...
                                    >
                                        ...
                                        {dressedInventory.map((p) => (
                                            ...
                                            >
                                                {' '}
                                                Harvested on{' '}
                                                {dayjs(p.HarvestDate).format(
                                                    'MMM D'
                                                )}{' '}
                                                ({p.QuantityRemaining} pcs
                                                remaining){' '}
                                            </Option>
                                        ))}{' '}
                                    </Select>{' '}
                                        ...
                                        () => ({
                                            validator(_, value) {
                                                if (
                                                    ...
                                                    value <=
                                                        selectedDressedProduct.QuantityRemaining
                                                ) {
                                                    return Promise.resolve();
                                                }
                                                return Promise.reject(
                    ...
                    <Card
                        ...
                        extra={
                            <Button
                                ...
                                onClick={() =>
                                    setIsManageTypesModalVisible(true)
                                }
                            >
                                {' '}
                                Manage Types{' '}
