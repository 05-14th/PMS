...
const Feedingandwatering: React.FC<FeedingandwateringProps> = ({ batchID }) => {
  ...
  const [medRelayState, setMedRelayState] = useState<{
    relay_med1: number;
    relay_med2: number;
    relay_med3: number;
  }>({ relay_med1: 0, relay_med2: 0, relay_med3: 0 });
  const [medicine, setMedicine] = useState<{
    med1: number;
    med2: number;
    med3: number;
  }>({ med1: 0, med2: 0, med3: 0 });
  ...
  React.useEffect(() => {
    // We keep the device IP fetching global as it affects all cages/batches
    axios
      .get(`${serverHost}/api/iot/manageDevices`)
      .then((response) => {
        ...
        devices.forEach((device: { deviceType: string; ipAddress: string }) => {
          if (device.deviceType === "Watering")
            newWateringIp = device.ipAddress;
          else if (device.deviceType === "Feeding")
            newFeedingIp = device.ipAddress;
          else if (device.deviceType === "Medicine")
            newMedicineIp = device.ipAddress;
        });

        setWateringIp(newWateringIp);
        setFeedingIp(newFeedingIp);
        setMedicineIp(newMedicineIp);

        // Only fetch telemetry if an IP is found (Global Telemetry Check)
        if (newWateringIp) {
          axios
            .get(`http://${newWateringIp}/telemetry`)
            .then((response) => {
  ...
  const handleWaterToggle = (relayNum: number) => {
    ...
    newState[`relay${relayNum}` as keyof typeof newState] = relayState[
      `relay${relayNum}` as keyof typeof newState
    ]
      ? 0
      : 1;
    setRelayState(newState);
    axios
      .post(`http://${wateringIp}/set-relays`, newState)
  ...
  const handleMedecine = (relayNum: number) => {
    ...
    newState[`relay_med${relayNum}` as keyof typeof newState] = medRelayState[
      `relay_med${relayNum}` as keyof typeof newState
    ]
      ? 0
      : 1;
    setMedRelayState(newState);
    axios
      .post(`http://${medicineIp}/set-relays`, newState)
          ...
          <div className="p-4 bg-white border-2 border-pink-200 shadow-sm rounded-xl">
            <div className="grid grid-cols-3 gap-3">
              {[1, 2, 3].map((num) => (
                <div key={num} className="flex flex-col">
                  <div className="mb-2 p-2 bg-gray-0  border border-pink-200 rounded-lg h-16 flex items-center justify-center text-xs text-gray-600 text-center">
                    Put the Medicine level Sensor Monitoring Here para sa
                    bahogan
                  </div>
                  <button
                    className={`aspect-square w-full flex items-center justify-center text-lg font-semibold transition border-2 rounded-full ${
                      isAutoMode
                        ? "bg-gray-100 border-gray-300 text-gray-400 cursor-not-allowed"
                        : medRelayState[
                              `relay_med${num}` as keyof typeof medRelayState
                            ]
                          ? "bg-green-200 border-green-500 text-green-900"
                          : "bg-white border-pink-200 text-green-700 hover:bg-green-100 active:bg-green-200"
                    }`}
