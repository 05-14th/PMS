...
const StockLevels: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const fetchDropdownData = async () => {
      try {
        const [suppliersRes, categoriesRes, unitsRes, subCategoriesRes] =
          await Promise.all([
            api.get("/api/suppliers"),
            api.get("/api/categories"),
            api.get("/api/units"),
            api.get("/api/subcategories"), // <-- ADD THIS
          ]);
        ...
        // --- FIX 2 (cont.): Set the new subcategories state ---
        setSubCategories(
      ...
      render: (_: any, record: PurchaseHistoryDetail) => {
        ...
        return (
          <Space>
            <Tooltip
              title={
                !isUntouched ? "Cannot edit a used purchase" : "Edit Purchase"
              }
            >
              <Button
                icon={<EditOutlined />}
